    ...
    <div class="auth-shell">
      ...
      <aside class="intro-panel">
        ...
        <div class="intro-body">
          <h1 class="intro-title">
            一个主题，<br />
            <span class="accent">一键生成</span>图文并茂的完整文章
          </h1>
          <p class="intro-desc">
        ...
        <div class="intro-foot">
          <span class="dot"></span>
          <span>注册即赠 <strong>100 积分</strong>，即刻开启创作之旅</span>
        </div>
      </aside>
      ...
      <main class="form-panel">
        <header class="card-head">
          <h2 class="card-title">创建你的<span class="accent">创作</span>账号</h2>
          <p class="card-subtitle">填写以下信息，快速完成注册</p>
        </header>
        ...
        <a-form
          ...
          @finish="handleSubmit"
        >
          <a-form-item name="userAccount" class="field">
            <a-input
          ...
          <a-form-item name="userPassword" class="field">
            <a-input-password
          ...
          <a-form-item name="checkPassword" class="field">
            <a-input-password
              ...
              autocomplete="new-password"
            >
              <template #prefix><SafetyCertificateOutlined class="field-icon" /></template>
            </a-input-password>
          </a-form-item>
...
// 校验规则与后端 UserRegisterRequest 约束对齐（min_length=4/8，max_length=256/512）
const rules = {
  userAccount: [
    { required: true, message: '请输入账号' },
    { min: 4, message: '账号长度不能小于 4 位' },
    { max: 256, message: '账号长度不能超过 256 位' },
    { pattern: /^\S(?:.*\S)?$/, message: '账号不能以空格开头或结尾' },
  ],
  userPassword: [
    { required: true, message: '请输入密码' },
    { min: 8, message: '密码长度不能小于 8 位' },
    { max: 512, message: '密码长度不能超过 512 位' },
  ],
  checkPassword: [
...
const handleSubmit = async (values: API.UserRegisterRequest) => {
  ...
  try {
    ...
    // 后端 422 校验失败或网络异常：提取 FastAPI detail，避免静默失败
    const detail = err?.response?.data?.detail
    if (Array.isArray(detail) && detail.length) {
      message.error('注册失败：' + detail.map((d: any) => d.msg).join('；'))
...
/* ===== 表单：扁平单框，不嵌套 ===== */
.reg-form {
  animation: rise-in 0.6s 0.24s both;
}
...
/* 让 ant-input / ant-input-affix-wrapper 直接作为单一边框输入框，不要嵌套边框 */
.reg-form :deep(.ant-input-affix-wrapper) {
  padding: 0;
...
.reg-form :deep(.ant-input-affix-wrapper .ant-input) {
  background: transparent;
...
.reg-form :deep(.ant-input-affix-wrapper .ant-input:hover) {
  border-color: var(--color-primary-light);
}

.reg-form :deep(.ant-input-affix-wrapper-focused .ant-input),
.reg-form :deep(.ant-input-affix-wrapper .ant-input:focus) {
  border-color: var(--color-primary) !important;
  box-shadow: 0 0 0 3px rgba(34, 197, 94, 0.12) !important;
}

.reg-form :deep(.ant-input-affix-wrapper .ant-input-prefix) {
  margin: 0 10px 0 14px;
  color: var(--color-text-muted);
}

.reg-form :deep(.ant-input-affix-wrapper-focused .ant-input-prefix) {
  color: var(--color-primary);
}

/* 没有 prefix 的纯 input */
.reg-form :deep(.ant-input:not(.ant-input-affix-wrapper-input)) {
  background: transparent;
  border: 1.5px solid var(--color-border);
  border-radius: 12px;
  min-height: 50px;
...
.reg-form :deep(.ant-form-item-explain-error) {
  font-size: 13px;
  margin-top: 2px;
}
