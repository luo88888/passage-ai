...
后续 M3 将在此扩展：预扣（USAGE_RESERVE）、结算（USAGE_SETTLE）、退补（USAGE_REFUND）、
按模型计价计算，以及更细粒度的并发控制（当前 grant_points 已通过行锁 + 流水保证一致性）。
"""
...
class PointsService:
    ...
    负责用户积分账户的增改查，所有余额变动均通过 grant_points 走「行锁更新 + 流水记账 +
