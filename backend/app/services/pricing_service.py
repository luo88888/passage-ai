...
负责按 model_pricing 配置把一段用量（UsageAccumulator 列表）换算成积分：
  ...
  - 匹配优先级：(category, provider, model, agentName) 精确 → (category, provider, model, '')
    → (category, provider, '*', '') 兜底；未命中按 config 中的系统默认单价（PricingConfig）
...
class PricingService:
    ...
    async def _calc_one(self, acc: UsageAccumulator) -> Decimal:
        ...
        if acc.category == "IMAGE":
            price_per_image = settings.default_image_price
        ...
        input_price = settings.default_llm_input_price
        output_price = settings.default_llm_output_price
        if pricing:
