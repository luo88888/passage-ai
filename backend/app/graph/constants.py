...
当前接入图的节点为 11 个：5 个副作用节点（bootstrap/confirm_title/confirm_outline/
ai_modify_outline/finalize）+ 6 个智能体节点（generate_title/generate_outline/generate_content/
image_analyzer/image_generator/merger）。副作用节点负责 DB 持久化/阶段流转/SSE 人机协同事件，
智能体节点只跑 agent + 发 agent 完成 SSE。interrupt_after 锚点设在 confirm_title /
confirm_outline / ai_modify_outline，保证「先持久化 + 发阶段事件再暂停」。
research / review / seo 为待实现占位（数据采集、内容审核暂不实现），不注册进 builder。
"""
# ==================== 当前接入图的节点名 ====================
...
NODE_RESEARCH = "research"   # 信息采集 Agent
